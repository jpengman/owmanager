...
public class SVGEndpoint {
	...
	public String test() {
		...
		Rectangle rect = new Rectangle(3, 3, 20, 20, 100, 100);
		LinearGradient lg = new LinearGradient("test", "0%", "0%", "0%", "100%");
		lg.addStop(new Stop("0%", "red"));
		lg.addStop(new Stop("100%", "blue"));
		rect.setGradient(lg);
		rect.setStroke("black");
		rect.setStrokeWidth("2");
		svg.addShape(rect);
		svg.addLinearGradient(lg);
		return svg.toString();
	}

	private String getSVG() {
		String svgStartTag = "<svg width='600' height='400'>\n";
	...
	private String[] getTankSVG(int x, int y, int width, int height, String type) {
		...
		for (Sensor sensor : findAllQuery.getResultList()) {
			...
			stopTags += "<stop offset='" + percent + "%' stop-color='rgb("
					+ ColorUtil.shaderBYR(sensor.getLastLoggedTemp(), 25, 45, 75) + ")' />\n";
			i++;
		}
